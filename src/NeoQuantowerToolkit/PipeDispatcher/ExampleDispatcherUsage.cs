...
public static class ExampleDispatcherUsage
{
    ...
    {
        await PipeDispatcher.Instance.Initialize();

        var subscription = PipeDispatcher.Instance.Subscribe<MyMessage>(async message =>
        {
            Console.WriteLine($"[Handler] Received message: {message.Text}");
            await Task.CompletedTask;
        }, tag: Tag);

        await PipeDispatcher.Instance.PublishAsync(new MyMessage
        {
